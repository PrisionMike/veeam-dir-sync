...
class Synchroniser:
    def __init__(self, source_path, replica_path, io_log_file, sync_log_file):
    ...
    def sync_file(self, dirpath, filename, file_hash):
        ...
        if not path.exists(replica_dir):
            ...
            log_dir_created(replica_dir, self.io_log_file)
        ...
        if not path.exists(replica_file_path):
            ...
            log_file_created(replica_file_path, file_hash, self.io_log_file)
        else:
            replica_file_hash = md5_file(replica_file_path)
            if replica_file_hash != file_hash:
                shutil.copyfile(source_file_path, replica_file_path)
                log_file_updated(replica_file_path, file_hash, self.io_log_file)
    ...
    def sync_dir(self, dirpath, dirname):
        ...
        if not path.exists(replica_subdir):
            ...
            log_dir_created(replica_subdir, self.io_log_file)
    ...
    def remove_dir(self, mem_path):
        log_dir_removed(mem_path, self.io_log_file)
        shutil.rmtree(mem_path)

    def remove_file(self, mem_path):
        log_file_removed(mem_path, self.io_log_file)
        remove(mem_path)